...
from exllamav2 import ExLlamaV2, ExLlamaV2Cache, ExLlamaV2Lora, ExLlamaV2Tokenizer
...
class ExLlamaV2StreamingGenerator(ExLlamaV2BaseGenerator):
    ...
    def __init__(
        ...
        self.no_logits = torch.empty(
            (0, ((self.model.config.vocab_size + 31) // 32) * 32), dtype=torch.float
        )
    ...
    def begin_stream(
        ...
        assert (
            input_ids.shape[0] <= 2
        ), "Streaming generator does not support batch size > 1"
        ...
        self.prob = torch.empty(
            (input_ids.shape[0], 0), device="cpu", dtype=torch.float
        )
    ...
    def _stream(self) -> (str, bool, torch.Tensor, torch.Tensor, torch.Tensor):
        ...
        if self.heal_next_token:
            ...
            old_tail = self.tokenizer.decode(
                self.sequence_ids[:, -self.tail_decode_tokens :]
            )[0]
            ...
            if self.first_token:
                self.settings.begin_filters(
                    self.tokenizer.get_id_to_piece_list()[last_token]
                )
                self.first_token = False

            # Regenerate the last token again, with prefix

            healed_token, _, eos, logits = self._gen_single_token(
            ...
            self.prob = torch.cat([self.prob, prob], dim=1)
        ...
        old_tail = self.tokenizer.decode(
            self.sequence_ids[:1, -self.tail_decode_tokens :]
        )[0]
        ...
        new_tail = self.tokenizer.decode(
            self.sequence_ids[:1, -(self.tail_decode_tokens + 1) :]
        )[0]
    ...
    def _catch_utf8(self, next_token, new_text):
        ...
        if self.expect_utf8:
            ...
            self.held_utf8_tokens = torch.cat(
                (self.held_utf8_tokens, next_token), dim=-1
            )
            self.expect_utf8 -= 1
            if self.expect_utf8 == 0:
    ...
    def _gen_begin(self, in_tokens, gen_settings):
        ...
        if self.draft_model is not None:
            ...
            self.draft_model.forward(
                self.sequence_ids[:1, :-1], self.draft_cache, preprocess_only=True
            )
    ...
    def _gen_feed_tokens(self, in_tokens, gen_settings):
        ...
        if self.draft_model is not None:
            self.draft_model.forward(
                self.sequence_ids[:, start:-1], self.draft_cache, preprocess_only=True
            )
            self.future_logits = None
            self.future_tokens = None
    ...
    def _gen_single_token(self, gen_settings, prefix_token=None):
        if self.draft_model is None:
            ...
            token, prob, eos = ExLlamaV2Sampler.sample(
                logits,
                gen_settings,
                self.sequence_ids[:1, :],
                random.random(),
            ...
            token, prob, eos, logits = self._gen_single_token_speculative(
    ...
    def _gen_single_token_speculative(self, gen_settings, prefix_token=None):
        if self.future_tokens is None:
            ...
            for k in range(self.num_speculative_tokens):
                logits = (
                    self.draft_model.forward(
                        draft_sequence_ids[:, -1:], self.draft_cache
                    )
                    .float()
                    .cpu()
                )
                token, prob, _ = ExLlamaV2Sampler.sample(
                    logits,
                    draft_gen_settings,
                    draft_sequence_ids,
                    random.random(),
            ...
            if self.sequence_ids.shape[0] > 1:
                self.future_tokens = draft_sequence_ids[
                    :, -1 - num_drafted_tokens :
                ].repeat(self.sequence_ids.shape[0], 1)
            else:
